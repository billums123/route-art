...
"    var seen = {};",
"",
"    // places already offered, so asking for more finds somewhere else",
"    var excl = [];",
"    if (optsO.exclude){",
"      for (var e5 = 0; e5 < optsO.exclude.length; e5++){",
"        excl.push([(optsO.exclude[e5][1] - G.P.lon0) * G.P.mPerLon,",
"                   (optsO.exclude[e5][0] - G.P.lat0) * G.P.mPerLat]);",
"      }",
"    }",
"    function excluded(x, y){",
"      for (var q5 = 0; q5 < excl.length; q5++){",
"        if (Math.hypot(x - excl[q5][0], y - excl[q5][1]) < (optsO.excludeRadius || 250)) return true;",
"      }",
"      return false;",
"    }",
"",
...
"",
"    var outRes = [];",
"    for (var fi = 0; fi < Math.min(found.length, 16); fi++){",
"      var F2 = found[fi];",
...
"      });",
"    }",
"    var widestClear = 0, bestOwner = -1;",
"    for (var w5 = 0; w5 < P.field.length; w5++){",
"      if (P.field[w5] > widestClear){ widestClear = P.field[w5]; bestOwner = P.owner[w5]; }",
"    }",
"    var fitS = Math.max(0, (widestClear - margin)) / Math.max(0.0001, radiusUnits);",
"    var bestSpace = G.polys[bestOwner];",
"    post({ type:'results', results: outRes, ms: Date.now() - t1, shapeLen: shapeLenO,",
"           openGround: true,",
"           fits: { maxLengthMeters: shapeLenO * fitS,",
"                   spanMeters: fitS * radiusUnits * 2,",
"                   place: bestSpace ? (bestSpace.name || bestSpace.kind) : null } });",
"    return;",
"  }",
"",
"  if (m.cmd === 'search'){",
...
"    var exclSt = [];",
"    if (opts.exclude){",
"      for (var e6 = 0; e6 < opts.exclude.length; e6++){",
"        exclSt.push([(opts.exclude[e6][1] - G.P.lon0) * G.P.mPerLon,",
"                     (opts.exclude[e6][0] - G.P.lat0) * G.P.mPerLat]);",
"      }",
"    }",
...
"            if (exclSt.length){",
"              var skip = false;",
"              for (var q6 = 0; q6 < exclSt.length; q6++){",
"                if (Math.hypot(cx - exclSt[q6][0], cy - exclSt[q6][1]) < (opts.excludeRadius || 500)){ skip = true; break; }",
"              }",
"              if (skip) continue;",
"            }",
"            var cost = sum / cnt;",
...
"    });",
"    post({ type:'results', results: results.slice(0, 12), ms: Date.now() - t0, shapeLen: shapeLen,",
